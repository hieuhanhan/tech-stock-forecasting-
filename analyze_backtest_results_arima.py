...
def daily_volatility(ret_simple: np.ndarray) -> float:
    """Realized daily volatility (std of simple returns)."""
...
def backtest_continuous_trace(
    ...
    for start in range(start_idx, n_total, retrain_interval):
        ...
        turnover_trace.extend(turnover.tolist())
    ...
    turnover_trace = np.array(turnover_trace, dtype=float)
    ...
    return dict(
        ...
        turnover_series=turnover_trace
    )
...
def cmd_viz(args):
    ...
    if args.annualize_turnover:
        # convert "mean turnover per step" to annualized (approx) by * 252 / window
    ...
    ax.set_title(ttl)
    ax.set_ylabel("Turnover" + (" (annualized)" if args.annualize_turnover else " (per-step mean)"))
    ax.set_xlabel("Time")
...
def cmd_sensitivity(args):
    ...
    if args.plot:
        ...
        fig, ax = plt.subplots(figsize=(6,4), dpi=140); im = ax.imshow(pv3.to_numpy(), aspect="auto")
        ax.set_xticks(range(len(pv3.columns))); ax.set_xticklabels(pv3.columns); ax.set_yticks(range(len(pv3.index))); ax.set_yticklabels(pv3.index)
        ax.set_xlabel("Cost multiplier"); ax.set_ylabel("Threshold multiplier"); ax.set_title("Sensitivity: Test Turnover"); fig.colorbar(im, ax=ax)
...
def cmd_cumret(args):
    ...
    if args.use_backtest:
        ...
        front = read_csv_required(Path(args.front_csv))
        need = {"fold_id","retrain_interval","front_type","p","q","threshold","sharpe","mdd"}
        if not need.issubset(front.columns):
            miss = need - set(front.columns)
            raise ValueError(f"front CSV missing required columns: {miss}")
        pick = pick_knee_from_front(front, args.fold_id, args.interval, args.front_type)
        if pick is None:
            raise RuntimeError("Knee not found in front CSV. Check fold/interval/front_type.")
        src_name = args.front_type
    ...
    # Rebuild simple return trace (uses same engine as viz/sensitivity)
    trace = backtest_continuous_trace(
        test_log=test_log,
        p=pick["p"], q=pick["q"], thr=pick["threshold"],
        retrain_interval=args.interval,
    ...
    # Cumulative return (simple returns)
    cumret = np.cumprod(1.0 + trace["ret_simple"]) - 1.0

    # Plot (%)
    fig, ax = plt.subplots(figsize=(8,4), dpi=140)
    ax.plot(100.0 * cumret)
    ax.set_title(f"Cumulative Return — fold {args.fold_id}, int {args.interval}, src {src_name}")
    ...
    meta = dict(
        ...
        source=src_name,
        params=dict(p=pick["p"], q=pick["q"], threshold=pick["threshold"]),
        cumulative_return=float(cumret[-1]) if cumret.size else 0.0
    )
    with open(outdir / "cumret_meta.json", "w") as f:
        json.dump(meta, f, indent=2)

    print(f"[OK] Saved cumulative_return.png to {outdir}")
    print(f"[OK] Final cumulative return = {meta['cumulative_return']:.6f}")

# =================================
# Subcommand: vol (realized & rolling volatility)
# =================================
def cmd_vol(args):
    setup_logging(args.debug)
    outdir = Path(args.outdir); outdir.mkdir(parents=True, exist_ok=True)
    ...
    # Rebuild trace (to get simple returns)
    trace = backtest_continuous_trace(
        test_log=test_log,
        p=pick["p"], q=pick["q"], thr=pick["threshold"],
        retrain_interval=args.interval,
    ...
    ax.plot(roll_vol)
    ax.set_title(f"Rolling Annualized Volatility (win={args.window}) — fold {args.fold_id}, int {args.interval}, src {src_name}")
...
def cmd_turnover(args):
    ...
    if args.use_backtest:
        ...
        need = {"fold_id","retrain_interval","front_type","p","q","threshold","sharpe","mdd"}
        if not need.issubset(front.columns):
            raise ValueError(f"front CSV missing required columns: {need - set(front.columns)}")
        pick = pick_knee_from_front(front, args.fold_id, args.interval, args.front_type)
        if pick is None:
            raise RuntimeError("Knee not found in front CSV.")
        src_name = args.front_type
    ...
    fig, ax = plt.subplots(figsize=(8,3.5), dpi=140)
    ax.plot(roll_turn)
    ttl = f"Rolling Turnover (win={args.turnover_window}) — fold {args.fold_id}, int {args.interval}, src {src_name}"
    if args.annualize_turnover: ttl += " (annualized)"
    ax.set_title(ttl)
    ax.set_ylabel("Turnover" + (" (annualized)" if args.annualize_turnover else " (per-step mean)"))
    ax.set_xlabel("Time")
    ...
    pd.DataFrame({
        "turnover_series": trace["turnover_series"],
        "rolling_mean": roll_turn
    }).to_csv(outdir / "turnover_series.csv", index=False)

    meta = dict(
        fold_id=args.fold_id,
        interval=args.interval,
        source=src_name,
        total_turnover=float(trace["turnover"])
    )
...
def build_parser():
    ...
    p_viz.add_argument("--outdir", default="viz_outputs/arima")
    ...
    p_sens.add_argument("--outdir", default="sensitivity_outputs/arima")
    ...
    # --- cumret subcommand ---
    p_cum = sub.add_parser("cumret", parents=[base_common], help="Cumulative Return only")
    p_cum.add_argument("--outdir", default="cumret_outputs/arima")
    p_cum.add_argument("--write-csv", action="store_true")
    p_cum.add_argument("--use-backtest", action="store_true",
                       help="If set, use --backtest-csv & --source-label; else use --front-csv & --front-type")
    p_cum.add_argument("--front-csv", help="(if not --use-backtest) Path to *_front.csv")
    p_cum.add_argument("--front-type", default="GA+BO", choices=["GA","GA+BO"])
    p_cum.add_argument("--backtest-csv", help="(if --use-backtest) Path to backtest_results_ga_vs_gabo.csv")
    p_cum.add_argument("--source-label", default="GA+BO_knee")

        # --- vol subcommand ---
    p_vol = sub.add_parser("vol", parents=[base_common], help="Realized & rolling volatility")
    p_vol.add_argument("--outdir", default="volatility/arima")
    p_vol.add_argument("--window", type=int, default=63, help="Rolling window for volatility")
    ...
    p_vol.add_argument("--use-backtest", action="store_true",
                       help="If set, use --backtest-csv & --source-label; else use --front-csv & --front-type")
    p_vol.add_argument("--front-csv", help="(if not --use-backtest) Path to *_front.csv")
    p_vol.add_argument("--front-type", default="GA+BO", choices=["GA","GA+BO"])
    p_vol.add_argument("--backtest-csv", help="(if --use-backtest) Path to backtest_results_ga_vs_gabo.csv")
    p_vol.add_argument("--source-label", default="GA+BO_knee")

    p_tov = sub.add_parser("turnover", parents=[base_common], help="Turnover plots only")
    p_tov.add_argument("--outdir", default="turnover_outputs/arima")
    ...
    p_tov.add_argument("--use-backtest", action="store_true",
                    help="If set, use --backtest-csv & --source-label; else use --front-csv & --front-type")
    p_tov.add_argument("--front-csv", help="(if not --use-backtest) Path to *_front.csv")
    p_tov.add_argument("--front-type", default="GA+BO", choices=["GA","GA+BO"])
    p_tov.add_argument("--backtest-csv", help="(if --use-backtest) Path to backtest_results_ga_vs_gabo.csv")
    p_tov.add_argument("--source-label", default="GA+BO_knee")
...
def main():
    ...
    if args.cmd == "viz":
        ...
        if args.use_backtest:
            if not args.backtest_csv:
                ap.error("cumret: --use-backtest requires --backtest-csv")
        else:
            if not args.front_csv:
                ap.error("cumret: without --use-backtest you must provide --front-csv")
        cmd_cumret(args)
    
    elif args.cmd == "vol":
        if args.use_backtest:
            if not args.backtest_csv:
                ap.error("vol: --use-backtest requires --backtest-csv")
        else:
            if not args.front_csv:
                ap.error("vol: without --use-backtest you must provide --front-csv")
        cmd_vol(args)
    
    elif args.cmd == "turnover":
        if args.use_backtest:
            if not args.backtest_csv:
                ap.error("turnover: --use-backtest requires --backtest-csv")
        else:
            if not args.front_csv:
                ap.error("turnover: without --use-backtest you must provide --front-csv")
        cmd_turnover(args)
