...
# ---------------- paths ----------------
BACKTEST_CSV = Path("tuning_results/backtest_arima/backtest_arima_results.csv")
OUT_DIR = Path("tuning_results/chap5/backtest_arima"); OUT_DIR.mkdir(parents=True, exist_ok=True)
FIG_A = OUT_DIR/"Fig_5_4a_arima_mdd_by_interval.png"
FIG_B = OUT_DIR/"Fig_5_4b_arima_mdd_overall.png"
...
need_cols = {"fold_id","retrain_interval","source","test_mdd"}
...
for interval in intervals:
    ...
    ga = sub[sub["source"] == "GA"]["test_mdd"].dropna().to_numpy()
    gabo = sub[sub["source"] == "GA+BO"]["test_mdd"].dropna().to_numpy()
    if ga.size > 0:
...
ax.set_ylabel("MDD Ratio (backtest)")
ax.set_title("MDD distribution by retraining interval — ARIMA (backtest)", fontsize=12, pad=8)
...
overall = [df[df["source"]=="GA"]["test_mdd"].dropna().to_numpy(),
           df[df["source"]=="GA+BO"]["test_mdd"].dropna().to_numpy()]
bp2 = ax.boxplot(overall, labels=["GA","GA+BO"], showmeans=True, patch_artist=True)
for patch, c in zip(bp2["boxes"], ["tab:blue","tab:orange"]):
    patch.set_facecolor(c); patch.set_alpha(0.35)

ax.set_ylabel("MDD Ratio (backtest)")
ax.set_title("MDD distribution — GA vs GA+BO (backtest)", fontsize=12, pad=8)
ax.grid(True, axis="y", linestyle=":", linewidth=0.6, alpha=0.6)
plt.tight_layout()
