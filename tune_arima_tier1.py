...
from arima_utils_cont import (
    Tier1ARIMAProblem,   # Tier1ARIMAProblem(train, val, normalize=False)
    arima_rmse,          # arima_rmse([p, q], train, val, normalize=False)
    LOG_DIR
)
...
def infer_base_dir_for_csvs(folds_path: Path) -> Path:
    """
    Try to infer 'data/processed_folds' from a typical path like:
    data/processed_folds/final/arima/arima_tuning_folds.json
    """
    try:
        return folds_path.parents[2].resolve()  # -> data/processed_folds
    except IndexError:
...
def main():
    ...
    for fid in tqdm(pending, desc='Tier1 ARIMA', leave=True, position=0, file=sys.stdout):
        ...
        try:
            ...
            # 1) Duyệt theo thế hệ (an toàn với nhiều version pymoo)
            for gen_idx, gen in enumerate(getattr(res_ga.algorithm, 'history', []), start=1):
                pop = getattr(gen, 'pop', None)
                if pop is None:
                    continue
                ...
                inds  = []
                for ind in pop:
                    if getattr(ind, 'X', None) is None or getattr(ind, 'F', None) is None:
                        continue
                    try:
                        p_, q_ = int(ind.X[0]), int(ind.X[1])
                        rmse_  = float(np.asarray(ind.F)[0])
                    except Exception:
                        continue
                    rmses.append(rmse_)
                    inds.append((p_, q_, rmse_))

                if not inds:
                    continue

                gen_best = float(np.min(rmses))
                for (p_, q_, rmse_) in inds:
                    ga_rows.append({
                        "stage": "GA",
                        "fold_id": int(fid),
                        "gen": int(gen_idx),
                        ...
                        "q": int(q_),
                        "rmse": float(rmse_),
                        "is_best_gen": bool(abs(rmse_ - gen_best) <= 1e-12),
                    })
                    best_rmse_so_far = min(best_rmse_so_far, rmse_)

            # 2) Fallback nếu history không cho gì: lấy current population
            if len(ga_rows) == 0:
                pop_now = getattr(res_ga.algorithm, 'pop', None) or getattr(res_ga, 'pop', None)
                if pop_now is not None and hasattr(pop_now, '__len__') and len(pop_now) > 0:
                    rmses = []
                    tmp   = []
                    for ind in pop_now:
                        if getattr(ind, 'X', None) is None or getattr(ind, 'F', None) is None:
                        ...
                        rmse_  = float(np.asarray(ind.F)[0])
                        rmses.append(rmse_)
                        tmp.append((p_, q_, rmse_))
                    if tmp:
                        gen_best = float(np.min(rmses))
                        for (p_, q_, rmse_) in tmp:
                            ga_rows.append({
                                "stage": "GA",
                                "fold_id": int(fid),
                                "gen": int(args.ngen),  # gán thế hệ cuối như fallback
                                "p": int(p_),
                                "q": int(q_),
                                "rmse": float(rmse_),
                                "is_best_gen": bool(abs(rmse_ - gen_best) <= 1e-12),
        ...
        # -------- Chọn top seeds duy nhất theo (p,q) --------
        ...
        if top_inds:
            ...
            # fallback nếu GA không cho seed hợp lệ
            x0 = [[1, 1]]
